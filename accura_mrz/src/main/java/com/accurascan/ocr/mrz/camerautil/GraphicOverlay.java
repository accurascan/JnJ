...
public class GraphicOverlay extends View {
  ...
  public abstract static class Graphic {
    ...
    public float deTranslateY(float y) {
      return deScaleY(y);
    }
    ...
    public float deTranslateX(float x) {
      if (overlay.facing == CameraSource.CAMERA_FACING_FRONT) {
        return overlay.getWidth() + deScaleX(x);
      } else {
        return deScaleX(x);
      }
    }
    ...
    public float deScaleY(float vertical) {
      return vertical / overlay.heightScaleFactor;
    }
    ...
    public float deScaleX(float horizontal) {
      return horizontal/overlay.widthScaleFactor;
    }
...
//    if (Util.isPortraitMode(getContext())) {
